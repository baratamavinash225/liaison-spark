class LiaisonBaseError(Exception):
...
class LLMRateLimitError(LiaisonBaseError):
    """Raised when the LLM provider returns a rate limit or quota error."""

    def __init__(
        self, message: str = "LLM rate limit exceeded. Please try again later."
    ):
        super().__init__(message, status_code=429)


class LLMProviderError(LiaisonBaseError):
...
class VectorDBError(LiaisonBaseError):
